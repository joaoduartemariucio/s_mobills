...
// Services
import 'package:s_mobills/core/networking/dio/dio_service.dart';
import 'package:s_mobills/core/networking/http/s_mobills_http_interface.dart';

// Exceptions
import 'package:s_mobills/core/networking/model/custom_exception.dart';

// Models
import 'package:s_mobills/core/networking/model/response_model.dart';

/// A service class implementing methods for basic API requests.
class SMobillsHTTPService implements SMobillsHTTPInterface {
  /// A public constructor that is used to initialize the API service
  SMobillsHTTPService({required this.dioService});
  ...
  /// in the **headers** of the request using an [AuthInterceptor].
  ...
  Future<List<T>> getCollectionData<T>({
    ...
    try {
      ...
      final data = await dioService.get<List<Object?>>(
        endpoint: endpoint,
        cacheOptions: dioService.globalCacheOptions?.copyWith(
          policy: cachePolicy,
  ...
  Future<T> getDocumentData<T>({
    ...
    try {
      // Entire map of response
      final data = await dioService.get<JSON>(
        endpoint: endpoint,
        queryParams: queryParams,
        cacheOptions: dioService.globalCacheOptions?.copyWith(
          policy: cachePolicy,
  ...
  Future<T> updateData<T>({
    ...
    try {
      // Entire map of response
      response = await dioService.patch<JSON>(
        endpoint: endpoint,
        data: data,
        options: Options(
          extra: <String, Object?>{
  ...
  Future<T> deleteData<T>({
    ...
    try {
      // Entire map of response
      response = await dioService.delete<JSON>(
        endpoint: endpoint,
        data: data,
        options: Options(
          extra: <String, Object?>{
